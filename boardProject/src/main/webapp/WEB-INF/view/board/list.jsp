	...
	<tr>
		...
		<td class="titled">
			<c:if test="${article.depth > 0}">
				<img src="<c:url value="/board/images/level.gif"/>" width="${5 * article.depth}">
				<img src="<c:url value="/board/images/re.gif"/>">
			</c:if>
			<c:if test="${article.depth == 0}">
				<img src="<c:url value="/board/images/level.gif"/>" width="${5 * article.depth}">
			</c:if>
			<a href="<c:url value="/board/content/${article.num},${currentPage}"/>">
				${article.subject}</a>
			<c:if test="${article.readcount >= 20}">
				<img src="<c:url value="/board/images/hot.gif"/>">
			</c:if>
		</td>
		<td>
